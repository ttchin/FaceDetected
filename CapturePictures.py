...
def capture_pictures_by_camera(num=200,
                               save_dir="./captured_pictures",
                               no_face_cropping=False,
                               no_capture_saving=False):
    """
    Capture pictures with faces detected and the cropped face images will be saved under
    "/cropped_faces" folder if face cropping function is enabled.
    ...
    Args:
        ...
        no_face_cropping (bool): If NOT to do face cropping and save the cropped face images under
                                 "cropped_faces" folder. Default: False
    ...
    while True:
        ...
        if frame_num % capture_every_n_frames == 0:
            ...
            if len(faces) > 0:
                ...
                if not no_capture_saving:
                    ...
                    frame_file_path = os.path.join(save_dir,
                                                   frame_file_name_with_ext)
                    cv2.imwrite(frame_file_path, frame)

                face_index = 1
                for (x, y, w, h) in faces:
                    if not no_face_cropping:
                        face_file_name_with_ext = frame_file_name + (
        ...
        cv2.putText(frame, ("FPS: %0.2f" % (frame_num /
                                            (time.time() - time_start))),
                    (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
        cv2.putText(frame, ("%d picture(s) captured!" % captured_frame_index),
    ...
    print("Now you could get the captured pictures under directory: " +
          save_dir)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Capture pictures with faces detected.')
    parser.add_argument(
        '-n', type=int, help='the number of pictures to capture. Default: 200')
    parser.add_argument(
        '-d',
        type=str,
        help=
        'the directory to save the captured pictures. Default: "./captured_pictures"'
    )
    parser.add_argument(
        '--no-face-cropping',
        action='store_const',
        default='False',
        const='True',
        dest='no_face_cropping',
        help='not to do the face cropping. Default: False')
    ...
    parser.add_argument(
        ...
        default='False',
        const='True',
        dest='no_capture_saving',
        help='not to store the captured pictures to save time. Default: False')
    ...
    # Start capturing
    capture_pictures_by_camera(args.n, args.d, args.no_face_cropping,
                               args.no_capture_saving)
